...
@pytest.mark.parametrize("kind, data, expected_error", [
    ('task', {'id': '123'}, "Task object missing required field: 'status.state'."),
    ('status-update', {'status': {}}, "StatusUpdate object missing required field: 'status.state'."),
    ('artifact-update', {}, "ArtifactUpdate object missing required field: 'artifact'."),
    ('artifact-update', {'artifact': {}}, "Artifact object must have a non-empty 'parts' array."),
    ('message', {'parts': []}, "Message object must have a non-empty 'parts' array."),
    ('message', {'parts': [{'text': 'hi'}], 'role': 'user'}, "Message from agent must have 'role' set to 'agent'."),
])
